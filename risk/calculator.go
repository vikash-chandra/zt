...
type PartialBookCostSLConfig struct {
	RiskRewardRatio float64 // e.g. 2.0 (1:2)
	PartialExitPct  float64 // e.g. 50.0%
	MoveSLToCost    bool    // e.g. true
	CostBufferPct   float64 // e.g. 0.05% (covers transaction charges)
	InitialSLMode   string  // "SETUP_BREAKOUT" or "PERCENTAGE"
	InitialSLPct    float64 // e.g. 1.5%
	SLBufferPct     float64 // e.g. 0.1%
}
...
type DynamicTrailingSLConfig struct {
	Stage1TriggerPct    float64 // default 0.3%
	Stage1TrailPct      float64 // default 0.05%
	Stage2TriggerPct    float64 // default 0.7%
	Stage2TrailPct      float64 // default 0.3%
	Stage3TriggerPct    float64 // default 1.2%
	Stage3TrailPct      float64 // default 0.6%
	Stage4TriggerPct    float64 // default 2.0%
	Stage4ExitPct       float64 // default 60.0%
	Stage4TrailPct      float64 // default 1.0%
	Stage5TriggerPct    float64 // default 2.5%
	StepTrailOffsetPct  float64 // default 0.6%
	TimeDecayMin        int     // default 45 min
	TimeDecayTriggerPct float64 // default 0.2%
	TimeDecayTrailPct   float64 // default 0.05%
	InitialSLMode       string  // "SETUP_BREAKOUT" or "PERCENTAGE"
	InitialSLPct        float64 // default 1.5%
	SLBufferPct         float64 // default 0.1%
}
