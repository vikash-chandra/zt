...
- **Low Volume (Option A)**: Lowest volume candle since 09:15 AM is Setup (Red for BUY, Green for SELL). Breakout valid **only on the immediate next candle** before `10:45:00 IST`.
- **Vande Bharat Momentum (4 Core Rules)**:
  1. **Rule 1 (Confirmation Breakout)**: Candle 2 breaks Master High (BUY) / Low (SELL) $\rightarrow$ Candle 2 is Confirmation Candle with SL at Candle 2 Low/High. Trade triggers when **Confirmation High / Low** is broken.
...
| `trail_sl_buffer_pct` | `20.0%` | 5-minute candle-close option price trailing buffer |
| `multiplier_on_reversal` | `true` | Multiplier lot scaling on trend reversal (1x $\rightarrow$ 2x $\rightarrow$ 3x $\rightarrow$ 4x) |
