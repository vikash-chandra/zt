...
### 16. Single-Day Chart Scoping & LightweightCharts Marker De-duplication
- **Single-Day API Scoping**: `/api/options/supertrends?date=YYYY-MM-DD` MUST filter candle responses strictly to the target date (`75 candles` starting at `09:15 AM IST`) when `date` query parameter is provided. This ensures that the UI chart canvas opens with the `09:15 AM IST` candle of that exact day as the left-most candle without displaying previous trading days.
