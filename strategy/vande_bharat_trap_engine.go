...
func (e *VandeBharatTrapEngine) TradeEndTime() string {
	e.mu.RLock()
	defer e.mu.RUnlock()
	if e.tradeEndTime == "" {
		return "11:00:00"
...
func (e *VandeBharatTrapEngine) SetTradeEndTime(t string) {
	e.mu.Lock()
	defer e.mu.Unlock()
	if t != "" {
...
// FakeMasterMaxPct returns the configured fake master candle max percentage
func (e *VandeBharatTrapEngine) FakeMasterMaxPct() float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.fakeMasterMaxPct
}

// MasterMaxPct returns the configured master candle max percentage
func (e *VandeBharatTrapEngine) MasterMaxPct() float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
...
func (e *VandeBharatTrapEngine) SLMinPct() float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.slMinPct
}
...
func (e *VandeBharatTrapEngine) SLMaxPct() float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
	return e.slMaxPct
}
...
func (e *VandeBharatTrapEngine) MasterMaxWickPct() float64 {
	e.mu.RLock()
	defer e.mu.RUnlock()
		...
		if isBuySetup {
			...
			if candle.High > master.High {
				...
				if candle.Close <= master.Low || candle.Close <= candle.Open {
					e.logger.Info("Invalidated VANDE_BHARAT_TRAP BUY setup: Candle 2 broke Master High but closed RED/DOJI (Shooting Star Rejection)",
						zap.String("symbol", symbol),
						zap.Float64("open", candle.Open),
						zap.Float64("close", candle.Close),
						zap.Float64("master_high", master.High),
			...
			if candle.Low < master.Low {
				...
				if candle.Close >= master.High || candle.Close >= candle.Open {
					e.logger.Info("Invalidated VANDE_BHARAT_TRAP SELL setup: Candle 2 broke Master Low but closed GREEN/DOJI (Hammer Rejection)",
						zap.String("symbol", symbol),
						zap.Float64("open", candle.Open),
						zap.Float64("close", candle.Close),
						zap.Float64("master_low", master.Low),
