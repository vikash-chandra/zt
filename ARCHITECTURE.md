...
2. **Configuration Loading**:
   * **File**: [config/settings.go](config/settings.go)
   * **Function**: `Load()` (Line 59)
   * *Loads API credentials and trading parameters from `.env`.*
3. **Database Connection**:
   * **File**: [data/database.go](data/database.go)
...
4. **Trading Bot Subsystems Assembly**:
   ...
   * **Line 69**: Creates the WebSocket ticker client via `data.NewRobustKiteTicker(...)` in [ticker.go](data/ticker.go#L33).
   * **Line 70–71**: Instantiates the 5-minute and 1-minute aggregators via `data.NewCandleAggregator(...)` in [candle_aggregator.go](data/candle_aggregator.go#L59).
   * **Line 74–75**: Instantiates the Kite Connect REST API client.
   * **Line 77**: Creates the `SecurityMaster` in [security_master.go](data/security_master.go#L40) to match constituent symbols to real tokens.
...
* **Function**: `Run()` (Line 122)
  ...
  * **Line 143**: Calls `tb.ticker.Connect(...)` in [ticker.go](data/ticker.go#L44) to open the WebSocket and subscribe to these instrument tokens.
...
1. **WebSocket Ingestion**:
   * **File**: [data/ticker.go](data/ticker.go)
...
3. **Candle Aggregation**:
   * **File**: [data/candle_aggregator.go](data/candle_aggregator.go)
...
2. **Low Volume Breakout Strategy**:
   * **File**: [strategy/low_volume_engine.go](strategy/low_volume_engine.go)
...
3. **Vande Bharat Breakout Strategy**:
   * **File**: [strategy/vande_bharat_engine.go](strategy/vande_bharat_engine.go)
   * **Functions**: `OnCandleClose(...)` and `CheckBreakout(...)`
...
4. **Pre-Trade Risk Management**:
   * **File**: [risk/risk_manager.go](risk/risk_manager.go)
...
2. **Order Placement**:
   * **File**: [execution/order_manager.go](execution/order_manager.go)
...
3. **Order Status Tracking**:
   * **File**: [main.go](main.go)
   * **Function**: `orderManagementLoop()` (Line 255)
   * **Line 271**: Starts tracking the order status by calling `tb.statusTracker.StartTracking(...)` in [status_tracker.go](execution/status_tracker.go#L52).
...
3. **Scheduler Integration & Force Recalculation**:
   * **File**: [scheduler.go](scheduler.go)
