...
// VandeBharatSelector implements Selector for the Vande Bharat strategy
type VandeBharatSelector struct {
	cfg *config.Settings
}

// NewVandeBharatSelector creates a new VandeBharatSelector instance
func NewVandeBharatSelector(cfg *config.Settings) *VandeBharatSelector {
	return &VandeBharatSelector{cfg: cfg}
}

// Name returns selector identity name
func (s *VandeBharatSelector) Name() string {
	return "VANDE_BHARAT_SELECTOR"
}

// SelectStocks runs sector calculations and stock percentage filters to return the watchlist
func (s *VandeBharatSelector) SelectStocks(ctx context.Context, logger *zap.Logger, client *kiteconnect.Client, secMaster *data.SecurityMaster, bias string, size int, maxPctChange float64) (map[string]int64, error) {
	kiteClient := client
	...
	for i := 0; i < finalSize; i++ {
		...
		logger.Info("Vande Bharat stock selected",
			zap.Int("rank", i+1),
